...
public class PositionsCommand implements Command {
    ...
    private void printPositions(JsonNode positions) {
        ...
        System.out.println("╠═════════╦════════════╦════════════╦═══════════╦══════════════════╣");
        System.out.println("║ Symbol  ║   Shares   ║   Avg Px   ║   Value   ║   Unrealized P/L ║");
        System.out.println("╠═════════╬════════════╬════════════╬═══════════╬══════════════════╣");

        for (JsonNode pos : positions) {
            String symbol = pos.get("symbol").asText();
            int shares = pos.get("shares").asInt();
            double avgPx = pos.get("averagePrice").asDouble();
            double value = pos.get("marketValue").asDouble();
            double pnl = pos.get("unrealizedPnl").asDouble();

            System.out.printf("║ %-7s ║ %10d ║ %10.2f ║ %9.2f ║ %16.2f ║%n", symbol, shares, avgPx, value, pnl);
        }
