...
public class FullPersistenceIntegrationTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            // IMPORTANTE: Cerrar cualquier instancia previa de RocksDB
            System.out.println("═══ Paso 0: Limpiando Recursos Previos ═══");
            closeExistingRocksDBInstance();
            System.out.println("✓ Recursos previos liberados\n");
            ...
            assert totalSessions >= 3 : "Should have at least 3 sessions, found: " + totalSessions;
            assert successfulLogins >= 2 : "Should have at least 2 successful logins, found: " + successfulLogins;
            assert failedLogins >= 1 : "Should have at least 1 failed login, found: " + failedLogins;
            ...
            if (todayStats.isPresent()) {
                ...
                System.out.println("⚠️  No se generaron estadísticas (puede que no haya datos suficientes)");
            }
            ...
            assert persistedSessions >= totalSessions : "Sessions should persist after restart. Expected: " + totalSessions + ", Found: " + persistedSessions;

            // Cerrar la nueva instancia
            newDbManager.close();
    ...
    /**
     * Cierra cualquier instancia existente de RocksDB usando reflection
     */
    private static void closeExistingRocksDBInstance() {
        try {
            Class<?> rocksDBManagerClass = Class.forName("com.boe.simulator.server.persistence.RocksDBManager");
            java.lang.reflect.Field instanceField = rocksDBManagerClass.getDeclaredField("instance");
            instanceField.setAccessible(true);

            Object instance = instanceField.get(null);
            if (instance != null) {
                java.lang.reflect.Method closeMethod = rocksDBManagerClass.getMethod("close");
                closeMethod.invoke(instance);
                instanceField.set(null, null);
                System.out.println("✓ RocksDB instance closed and reset");
            }
        } catch (Exception e) {
            // Silently ignore if no instance exists
        }
    }
